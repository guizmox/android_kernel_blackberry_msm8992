...
unsigned int divide_rounded(unsigned int dividend, unsigned int divisor)
{
	unsigned long long dividend_shifted = dividend << 12;
	unsigned long long divisor_shifted = divisor << 12;
	
	if (dividend > 0xFFFFF || divisor > 0xFFFFF) {
		pr_err("%s ERROR: Values are too big!\n", __func__);

		return 0;
	}

	return ((unsigned int)(((dividend_shifted << 13) / divisor_shifted + 1) >> 1) + (((((dividend_shifted << 13) / divisor_shifted + 1) >> 12) & 1) << 12)) >> 12;
}

static char brightness_b5[6] = { 0xb5, 0x50, 0x00, 0x19, 0x00, 0x45 };
static char brightness_set_off_ratio[3] = { 0xb6, 0x98, 0x00 };
static char brightness_set_elvss_voltage[3] = { 0xb2, 0x00, 0x00 };
static char brightness_gamma_correction[36] = {	0xca, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00 };

static struct dsi_cmd_desc backlight_cmds[] = {
	{ {DTYPE_DCS_LWRITE, 1, 0, 0, 0, 3}, (char []) {0xf0, 0x5a, 0x5a} },
	{ {DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(brightness_b5)}, brightness_b5 },
	{ {DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(brightness_set_off_ratio)}, brightness_set_off_ratio },
	{ {DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(brightness_set_elvss_voltage)}, brightness_set_elvss_voltage },
	{ {DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(brightness_gamma_correction)}, brightness_gamma_correction },
	{ {DTYPE_DCS_LWRITE, 1, 0, 0, 1, 2}, (char []) {0xf7, 0x03} }, //  gamma/aor update
...
static struct dsi_cmd_desc alpm_brightness_cmd[] = {
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, 3}, (char []) {0xf0, 0x5a, 0x5a}},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, 2}, (char []) {0xb0, 0x05}},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, 4}, NULL},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 1, 2}, (char []) {0xf7, 0x03}}, //  gamma/aor update
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, 2}, (char []) {0xf7, 0x00}},
	{{DTYPE_DCS_LWRITE, 1, 0, 0, 0, 3}, (char []) {0xf0, 0xa5, 0xa5}}
};
...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (pinfo->blank_state == MDSS_PANEL_BLANK_LOW_POWER) {
		...
		for (i = 0; i < NUM_ALPM_BRIGHTNESS_BB_VALUES; i++) {
			int level_nits = level * 600 / 255;
			if (level_nits < 10 * i + 15)
				break;
		}

		alpm_brightness_cmd[2].payload = alpm_brightness_bb_values[i];
		...
		*/
		...
		if (level > 31)
			...
			b5_value = 107 - 2 * level;
		
		if (level > 349)
			off_ratio = 22 - divide_rounded(9 * (level - 350), 250);
		else if (level > 149)
			off_ratio = 23 - divide_rounded(level - 150, 200);
		else if (level > 0)
			off_ratio = divide_rounded(5 * (level - 1), 149) + 18;
		
		if (level > 350)
			elvss_voltage = 608 - 249 * level / 250;
		else if (level > 49)
			elvss_voltage = 259;
		else if (level > 5)
		...
		*/
		...
		brightness_set_elvss_voltage[1] = elvss_voltage;
		brightness_set_elvss_voltage[2] = (elvss_voltage >> 4) & 0xF0;
